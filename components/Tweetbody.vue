<template>
  <div>
    ...
    >
      <v-card-title class="icons">
        ...
        <div class="text-center">
          <v-dialog v-model="dialog" width="50%" :retain-focus="false">
            ...
            <v-card>
              <v-card-title class="text-h5 grey lighten-2">ツイート編集</v-card-title>
              <v-card-text>
                <v-text-field placeholder="編集する内容" v-model="editTweetData"></v-text-field>
              </v-card-text>
              <v-divider></v-divider>
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="primary" text @click="editTweet(index)">編集を完了する</v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
        </div>
      ...
      <v-card-text class="headline font-weight-bold" @click="moveToReplyPage(tweet.id)">
        <p class="tweet">{{tweet.message}}</p>
      </v-card-text>
      <!-- ツイートに添付される画像 -->
      <div @click="moveToReplyPage(tweet.id)">
        <v-card width="30%" class="mx-auto" v-show="tweet.tweetImage">
          <img :src="tweet.tweetImage" class="object-cover border mx-auto" />
        </v-card>
      </div>
      <!-- <div v-else>
      ...
      <v-card-actions>
        <v-list-item class="grow">
          ...
          >
            <v-img class="elevation-6" alt :src="tweet.photoURL"></v-img>
          </v-list-item-avatar>
          <!-- :src="currentImg" -->
          ...
          <v-row align="center" justify="end">
            ...
            <span class="subheading mr-2">{{tweet.good}}</span>
            <span class="mr-1">·</span>
            <v-icon class="mr-1">
              <!-- mdi-share-variant -->
            </v-icon>

            <span class="subheading">{{tweet.date}}</span>
          </v-row>
        </v-list-item>
      </v-card-actions>
    </v-card>
    ...
    async editTweet(index) {
      ...
      if (this.editTweetData == null) {
        ...
        await this.$firestore
          .collection("tweets")
          .doc(this.editId)
          .update({ message: this.editTweetData })
          .then(() => {
