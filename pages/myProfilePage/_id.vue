<template>
  <div>
    ...
    <div class="profile-icon">
      <v-avatar color="grey" size="240" @click="selectImage" style="cursor:pointer;">
        <template v-if="user.photoURL">
          <img :src="user.photoURL" class="w-60 h-60 object-cover border rounded-full" />
        </template>

        <template v-else-if="postData.thumbnail">
          <img :src="postData.thumbnail" class="w-60 h-60 object-cover border rounded-full" />
        </template>

        <template v-else>
          <i class="material-icons text-9xl text-white">person</i>
        </template>
        <input ref="image" type="file" style="display: none" accept="image/*" @change="changeImg" />
      </v-avatar>
      <p class="profile-text">{{currentUser}}</p>
    </div>

    <div class="tabs-section">
      <v-toolbar color="white" dark flat prominent>
        <!-- <v-text-field
        ...
        <template v-slot:extension>
          <v-tabs v-model="tabs" centered>
            <v-tab class="v-tab">
              <p style="color:grey;">Tweet</p>
            </v-tab>
            <v-tab class="v-tab">
              <p style="color:grey;">Follow</p>
            </v-tab>
            <v-tab class="v-tab">
      ...
      <v-tabs-items v-model="tabs">
        ...
        <v-tab-item>
          <v-card flat>
            ...
            <v-card-text>
              <p>Maecenas ullamcorper, dui et placerat feugiat, eros pede varius nisi, condimentum viverra felis nunc et lorem. Sed hendrerit. Maecenas malesuada. Vestibulum ullamcorper mauris at ligula. Proin faucibus arcu quis ante.</p>
