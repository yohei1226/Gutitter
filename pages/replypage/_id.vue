  ...
  <div>
    ...
    >
      <v-card-text class="headline font-weight-bold">
        <p class="tweet">{{tweetData.message}}</p>
      </v-card-text>
      <!-- ツイートに添付される画像 -->
      <v-card width="30%" class="mx-auto" v-show="tweetData.tweetImage">
        <img :src="tweetData.tweetImage" class="object-cover border mx-auto" />
      </v-card>
      <!-- <div >
      ...
      <v-card-actions>
        <v-list-item class="grow">
          <v-list-item-avatar color="grey darken-3" size="62">
            <v-img class="elevation-6 replypage-hovers" alt :src="tweetData.photoURL"></v-img>
          </v-list-item-avatar>
          <!-- :src="currentImg" -->
          ...
          <v-row align="center" justify="end">
            <v-icon class="mr-1" color="pink" @click="goodButton">mdi-heart</v-icon>
            <span class="subheading mr-2">{{tweetData.good}}</span>
            <span class="mr-1">·</span>
            <v-icon class="mr-1">
              <!-- mdi-share-variant -->
            </v-icon>

            <span class="subheading">{{tweetData.createdAt}}</span>
          </v-row>
        </v-list-item>
      </v-card-actions>
    </v-card>
    ...
    <div class="pt-10 mx-auto tweet-input mb-3 mt-16" style="display:flex">
      ...
      <v-btn depressed color="indigo" class="mt-3 ml-10 tweet-button" @click="pushToFirestore">reply</v-btn>
    </div>
    ...
    >
      <template>
        <v-subheader v-if="reply.header" :key="reply.header" v-text="reply.header"></v-subheader>

        <v-divider v-else-if="reply.divider" :key="index" :inset="reply.inset"></v-divider>

        <v-list-item v-else :key="reply.title">
...
export default {
  ...
  methods: {
    ...
    async goodButton() {
      ...
      let tweet = await this.$firestore
        .collection("tweets")
        .doc(this.goodId)
        .get();
      // debugger
